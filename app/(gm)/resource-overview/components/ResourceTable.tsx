        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider group cursor-help relative text-nowrap">
                Working Hours
                    ...
                    <div className="group relative">
                      <div className="flex items-center gap-3">
                        <div className="flex-1 w-32 bg-gray-200 rounded-full h-3">
                          <div
                            className={`h-3 rounded-full transition-all ${workload > 100
                              ? 'bg-red-500'
                              : workload > 70
                                ? 'bg-orange-500'
                                : workload > 40
                        ...
                        <span
                          className={`text-sm font-bold whitespace-nowrap min-w-[50px] text-right ${workload > 100
                            ? 'text-red-600'
                            : workload > 70
                              ? 'text-orange-600'
                              : workload > 40
                  ...
                  <td className="px-6 py-4">
                    <div className="group relative inline-block cursor-help">
                      ...
                      >
                        ...
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${status === 'available'
                            ? 'bg-green-500'
                            : status === 'moderate'
                              ? 'bg-yellow-500'
                              : status === 'busy'
                      ...
                      <div className="absolute left-0 top-full mt-1 w-72 bg-gray-900 text-white text-xs rounded-lg p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 shadow-lg">
                        {status === 'available' && 'Low workload (≤ 40%), resource has high availability'}
                        {status === 'moderate' && 'Balanced workload (41–70%)'}
                        {status === 'busy' && 'High workload (71–100%), near full capacity'}
                        {status === 'overloaded' && 'Work exceeds 8 hours/day (>100%), needs attention'}
                      </div>
                    </div>
                  </td>
                </tr>
