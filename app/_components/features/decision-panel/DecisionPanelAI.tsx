...
interface DecisionPanelAIProps {
...
export function DecisionPanelAI({ selectedProject }: DecisionPanelAIProps) {
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
        <div className="flex flex-col items-center justify-center text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Decision Panel</h3>
          <p className="text-sm text-gray-600 mb-1">AI-assisted resource planning decisions</p>
          <p className="text-xs text-gray-500 max-w-md mt-2">
            Click on a project in the Gantt chart to see AI-powered recommendations and make decisions
          </p>
        </div>
      </div>
    );
  ...
  const resourceGap = selectedProject.requiredResources - selectedProject.assignedResources.length;
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-br from-blue-600 to-green-600 rounded-xl p-6 text-white shadow-lg">
        <div className="flex items-start gap-3">
          <div className="p-2 bg-white/20 rounded-lg backdrop-blur">
            <Zap className="w-6 h-6" />
          </div>
          <div className="flex-1">
            <h2 className="text-xl font-semibold mb-1">Decision Panel</h2>
            <p className="text-blue-100 text-sm">AI-assisted resource planning decisions</p>
          </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span
                className={`w-2.5 h-2.5 rounded-full ${selectedProject.status === 'on-track'
                    ? 'bg-green-500'
          ...
          <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-200">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-900">
                {selectedProject.assignedResources.length} / {selectedProject.requiredResources}
              </p>
            </div>
            ...
            <div>
              ...
              <p
                className={`text-sm font-medium ${selectedProject.resourceUtilization > 100 ? 'text-red-600' : 'text-green-600'
                  }`}
              >
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        ...
        <div className="space-y-3">
          {contractDecisions.map((decision) => (
            <div key={decision.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="flex items-start gap-3 mb-3">
                <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-yellow-500 rounded-full flex items-center justify-center flex-shrink-0">
                  <span className="text-white text-sm font-medium">{decision.employeeAvatar}</span>
                </div>
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-semibold text-gray-900">{decision.employeeName}</p>
                      <p className="text-xs text-gray-600 mt-1">{decision.currentWorkload}</p>
                    </div>
                    <span
